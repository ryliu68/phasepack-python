...
def solveWirtFlow(A=None, At=None, b0=None, x0=None, opts=None, *args, **kwargs):
    innerOpts = struct
    ...
    def updateObjective(z=None):
        f = dot(0.5, norm(abs(z) ** 2 - b0 ** 2) ** 2)
        gradf = np.multiply((abs(z) ** 2 - b0 ** 2), z)
        return f, gradf

    # def updateObjective(__=None,__=None,*args,**kwargs):
    #     f=lambda z=None: dot(0.5,norm(abs(z) ** 2 - b0 ** 2) ** 2)
    #     gradf=lambda z=None: np.multiply((abs(z) ** 2 - b0 ** 2),z)
    #     return f,gradf

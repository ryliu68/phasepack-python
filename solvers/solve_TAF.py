...
def solveTAF(A=None, At=None, b0=None, x0=None, opts=None, *args, **kwargs):
    ...
    def updateObjective(__=None, Ax=None, *args, **kwargs):
        ...
        mask = abs(Ax) > dot(1 / (1 + opts.gamma), b0)

        s = sum(mask)
        f = lambda z=None: dot(
            (0.5 / s), sum(np.multiply(mask, (abs(z) - b0) ** 2)))
        gradf = lambda z=None: np.multiply(
            dot((1.0 / s), mask), (z - np.multiply(b0, np.sign(z))))
        return f, gradf
    return sol, outs

...
def solveAmplitudeFlow(A=None, At=None, b0=None, x0=None, opts=None, *args, **kwargs):
    ...
    def updateObjective():
        ...
        def f(z=None):
            return dot(1 / 2, norm(abs(z) - b0) ** 2)

        def gradf(z=None):
            return (z - np.multiply(b0, np.sign(z)))

    # def updateObjective(__=None,__=None,*args,**kwargs):
    #     f=lambda z=None: dot(1 / 2,norm(abs(z) - b0) ** 2)
    #     gradf=lambda z=None: (z - np.multiply(b0,np.sign(z)))
    #     return f,gradf
    return sol, outs

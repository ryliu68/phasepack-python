...
import numpy as np
import struct
from util.build_test_problem import buildTestProblem
from util.plot_error_convergence import plotErrorConvergence
from util.plot_recovered_VS_original import plotRecoverdVSOriginal
...
#  accuracy of the solver.
# reconError = norm(xt - x) / norm(xt)
reconError = math.sqrt(np.dot((xt - x), (xt - x))) / math.sqrt(np.dot(xt, xt))
print('relative recon error = %d\n', reconError)
# Plot a graph of error(definition depends on if opts.xt is provided) versus
# the number of iterations.
plotErrorConvergence(outs, opts)
